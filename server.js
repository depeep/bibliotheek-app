...
// Endpoints voor uitleenbeheer 7 januari 2026

// ---- Hulp:  validatie van de datum---- MOET VOOR HET POST endpoint staan anders werkt het niet
...
// ---- Endpoint: POST /uitleen ---- uitleen registratie
...
//GET /uitleen met datum formatting en actief als nummer
...
// endpoint om alle uitgeleende boeken met lenernaam op te halen  
// in de query alleen actieve uitleen (actief = 1) en een CAST om actief als nummer te krijgen
//GET /uitleen met datum formatting en actief als nummer
app.get('/uitleen/terugbrengen', (req, res) => {
  const { lenerid } = req.query;
  let sql = `
  SELECT uitleen.transactieid, boeken.afbeelding, boeken.titel, leners.naam AS lener_naam, uitleen.datum, CAST(uitleen.actief AS UNSIGNED) AS actief 
  FROM uitleen 
  INNER JOIN boeken ON boeken.id = uitleen.boekid 
  INNER JOIN leners ON leners.id = uitleen.lenersid 
  WHERE uitleen.actief=1`;
...
// Endpoint om een uitgeleend boek in te nemen (actief op 0 zetten) op transactieid

app.patch('/uitleen/innemen/:id', (req, res) => {
    const { id } = req.params;
    const transactieId = parseInt(id, 10); // Zorg dat het een nummer is want in de databes staat bit(1) en dat werkt alleen met int of echte 0

    if (!transactieId) {
        return res.status(400).json({ message: 'Geen geldig transactieid opgegeven' });
    }
    
    const sql = `UPDATE uitleen SET actief = b'0' WHERE transactieid = ?`; //binary 0 voor bit(1)
    db.query(sql, [transactieId], (err, result) => {
      if (err) {
          return res.status(500).json({ message: 'Database fout', error: err.message });
      }
      if (result.affectedRows === 0) {
          return res.status(404).json({ message: 'Uitleen niet gevonden' });
      }
      if (result.changedRows === 0) {
          return res.status(200).json({ message: 'Geen wijziging: boek was al ingenomen' });
      }

    res.json({ message: 'Boek ingeleverd en uitleen bijgewerkt' });
});
