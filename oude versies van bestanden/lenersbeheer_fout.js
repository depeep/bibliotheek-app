...
async function addOrUpdateLener() {
  ...
  const lenerData = { id, naam, actief };
...
function vulFormulier(id, naam, actief) {
  ...
  //vullen
  idLener= id; //toegevoegd om lener id op te slaan zonder dat het in formulier staat
...
const apiUrl = "http://localhost:3000/leners"; // Backend endpoint
let idLener = ''; // ID van lener wordt hier opgeslagen
...
async function addOrUpdateLener() {
  const id = idLener; // Gebruik variabele, niet formulier veld
  const naam = document.getElementById("lenerNaam").value.trim();
  const actief = parseInt(document.getElementById("lenerActief").value);
  ...
  // Alleen naam en actief in body
  const lenerData = { naam, actief };

  try {
    let response;
    if (id) {
      console.log(`PATCH lener met ID: ${id}`);
      response = await fetch(`${apiUrl}/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(lenerData)
    ...
    if (!response.ok) throw new Error("Fout bij opslaan");
    const result = await response.json();
    alert(`Lener succesvol opgeslagen! ${result.id ? "ID: " + result.id : ""}`);
    clearForm();
    searchLener(); // Vernieuw lijst
  } catch (error) {
    alert("Er is een fout opgetreden: " + error.message);
