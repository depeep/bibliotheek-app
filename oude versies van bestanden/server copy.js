...
/*
// Endpoint om een boek bij te werken op id
app.put('/boeken/:id2', (req, res) => {
  const { id } = req.params;
  const { titel, auteur, uitgeverij, publicatiejaar, exemplaren } = req.body;

  const sql = `
    UPDATE boeken 
    SET titel = ?, auteur = ?, uitgeverij = ?, publicatiejaar = ?, exemplaren = ?
    WHERE id = ?
  `;

  db.query(sql, [titel, auteur, uitgeverij, publicatiejaar, exemplaren, id], (err, result) => {
    if (err) return res.status(500).json({ error: err });
    ...
    res.json({ message: 'Boek succesvol bijgewerkt' });
  });
});
*/
