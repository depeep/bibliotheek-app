...
/* co-pilot verbeterde slaOp functie met betere foutafhandeling en UI reset 
...
async function slaOp() {
  toonMelding(`Opslaan functie gestart voor boekId: ${boekId} en lenerId: ${lenerId}`);

  // Validatie
  if (!boekId) {
    toonMelding("Boek selecteren is verplicht!");
    return;
  }
  if (!lenerId) {
    toonMelding("Lener selecteren is verplicht!");
    return;
  }

  // Bouw payload — maak desnoods numeriek als backend dit verwacht
  const uitleenData = {
    boekid: Number.isNaN(Number(boekId)) ? boekId : Number(boekId),
    lenersid: Number.isNaN(Number(lenerId)) ? lenerId : Number(lenerId),
    datum: new Date().toISOString().split('T')[0], // YYYY-MM-DD
    actief: 1
  };
  console.log("Uitleen data om op te slaan:", uitleenData);

  // Optioneel: voorkom dubbele submits
  const opslaanBtn = document.getElementById('opslaanBtn');
  if (opslaanBtn) opslaanBtn.disabled = true;

  try {
    const response = await fetch(apiUitleenUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(uitleenData)
    });

    // Controleer HTTP status direct hier, binnen try
    if (!response.ok) {
      // Probeer backend-foutboodschap te lezen
      let details = '';
      try {
        const err = await response.json();
        details = err?.message || JSON.stringify(err);
      } catch {
        // response body is geen JSON of leeg
      }
      throw new Error(`Fout bij opslaan uitleen (status ${response.status})${details ? `: ${details}` : ''}`);
    }

    const result = await response.json();
    toonMelding(`Uitleen succesvol opgeslagen! Nieuw ID: ${result?.id ?? '(onbekend)'}`);

    // Alleen bij succes: UI resetten
    document.getElementById("geselecteerdeBoek")?.innerHTML = "";
    document.getElementById("geselecteerdeLener")?.innerHTML = "";
    boekId = '';
    lenerId = '';
    ...
    console.error("Fout bij het opslaan van uitleen:", error);
    toonMelding("Er is een fout opgetreden bij het opslaan van de uitleen.\n" + (error?.message ?? 'Onbekende fout'));
  } finally {
