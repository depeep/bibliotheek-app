...
app.post('/boeken', (req, res) => {
  const { isbn13, isbn10, titel, auteur, uitgeverij, beschrijving, publicatiejaar, afbeelding, taal, paginas } = req.body;

  const sql = `
    INSERT INTO boeken 
    (isbn13,isbn10, titel, auteur, uitgeverij, beschrijving, publicatiejaar, afbeelding, taal, paginas) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;

  db.query(sql, [isbn13, isbn10, titel, auteur, uitgeverij, beschrijving, publicatiejaar, afbeelding, taal, paginas], (err, result) => {
    if (err) {
